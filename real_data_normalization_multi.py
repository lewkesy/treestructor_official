...
def centerailze_real_data_axis(pc, idx):

    # pc_mean = (np.max(pc[:, :3], axis=0) + np.min(pc[:, :3], axis=0)) / 2
    # pc[:, :3] -= pc_mean
    # pc[:, :3] -= np.array([0, pc[:, 1].min(), 0])
    # pc[:, :3] = pc[:, :3] / pc[:, 1].max() * 6
    # pc[:, 3] = -pc[:, 3]
    
    if pc.shape[1] == 3:
        save_ply('./data/normalized_%d_'%idx + filename, pc)
    else:
        save_ply_with_color('./data/normalized_%d_'%idx + filename, pc)
...
if filename.split('.')[-1] == 'ply':
    ...
    z = data["vertex"]["z"]
    # embed()
    # idx = data['vertex']['scalar_Scalar_field']
    idx = data['vertex']['scalar_treeID']
...
for i, key in enumerate(idx):
    if key not in trees:
        trees[key] = []
    trees[key].append(pc[i])

for i, key in enumerate(tqdm(trees.keys())):
    pc = np.array(trees[key])
    centerailze_real_data_axis(pc, i)
